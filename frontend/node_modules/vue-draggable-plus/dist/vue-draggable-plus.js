...
import { getCurrentInstance as ft, unref as G, watch as an, onUnmounted as ln, onMounted as sn, nextTick as Yt, isRef as $e, defineComponent as un, computed as yt, toRefs as cn, ref as fn, reactive as dn, h as hn, isProxy as pn } from "vue";
...
function tn(...t) {
  var Z, L;
  const e = (Z = ft()) == null ? void 0 : Z.proxy, n = t[0];
  let [, o, r] = t;
  Array.isArray(G(o)) || (r = o, o = null);
  let i = null;
  const {
    immediate: a = !0,
    clone: l = Qn,
  ...
  function u(v) {
    var N;
    const T = G((N = G(o)) == null ? void 0 : N[v.oldIndex]), D = l(T);
    Ft(T, D), v.item[Rt] = D;
  }
  function d(v) {
    const T = v.item[Rt];
  ...
  function F() {
    var N;
    const P = (N = G(r)) != null ? N : {}, { immediate: v, clone: T } = P, D = Ve(P, ["immediate", "clone"]);
    return _t(D, (V, W) => {
      Tn(V) && (D[V] = ($, ...se) => {
        const Q = no();
        return Cn($, Q), W($, ...se);
...
const ct = [
  ...
  setup(t, { slots: e, emit: n, expose: o, attrs: r }) {
    const i = ct.reduce((d, f) => {
      ...
      const y = cn(t), { modelValue: d } = y, f = Ve(y, ["modelValue"]), m = Object.entries(f).reduce((b, [w, F]) => {
        const X = G(F);
  ...
  [Xt.mounted](t, e) {
    const n = pn(e.value) ? [e.value] : e.value, o = tn(t, ...n);
    it.set(t, o.destroy);
  },
  [Xt.unmounted](t) {
    var e;
