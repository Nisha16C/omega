...
function createRequestState(hookType, methodHandler, useHookConfig, initialData, immediate = shared.falseValue, watchingStates, debounceDelay = 0) {
    ...
    const data = create((shared.isFn(initialData) ? initialData() : initialData), 'data');
...
var usePagination = (handler, config = {}) => {
    ...
    const { preloadPreviousPage = shared.trueValue, preloadNextPage = shared.trueValue, total: totalGetter = res => res.total, data: dataGetter = res => res.data, append = shared.falseValue, initialPage = 1, initialPageSize = 10, watchingStates = [], initialData, immediate = shared.trueValue, middleware = shared.noop, force = shared.noop, ...others } = config;
    ...
    const canPreload = async (payload) => {
        ...
        if (expireMilliseconds(shared.MEMORY) <= shared.getTime()) {
            return shared.falseValue;
        }
        if (forceRequest) {
            return shared.trueValue;
    ...
    const fetchNextPage = async (rawData, force = shared.falseValue) => {
        ...
        if (preloadNextPage &&
            ...
            shared.promiseCatch(fetch(fetchMethod, force), shared.noop);
        }
    };
    ...
    const fetchPreviousPage = async (rawData) => {
        ...
        if (preloadPreviousPage &&
            ...
            shared.promiseCatch(fetch(fetchMethod), shared.noop);
        }
    };
    ...
    const remove = (...positions) => {
        ...
        return add2AsyncQueue(async () => {
            ...
            if (fillingItemsLen > 0 || isLastPageVal) {
                ...
                if (!append && isLastPageVal && shared.len(newListData) <= 0) {
                    page.v = pageVal - 1;
                }
...
const actionDelegationMiddleware = (id) => {
    ...
    const delegated = ref(shared.falseValue);
    return (context, next) => {
        ...
        if (!delegated.current) {
            ...
            delegated.current = shared.trueValue;
        }
        return next();
    };
};
...
const accessAction = (id, onMatch, silent = false) => {
    ...
    if (typeof id === 'symbol' || shared.isString(id) || shared.isNumber(id)) {
        actionsMap[id] && shared.pushItem(matched, ...actionsMap[id]);
    }
        ...
        shared.forEach(shared.filterItem(shared.objectKeys(actionsMap), idItem => id.test(idItem)), idItem => {
            shared.pushItem(matched, ...actionsMap[idItem]);
        });
    }
    ...
    shared.forEach(matched, onMatch);
};
