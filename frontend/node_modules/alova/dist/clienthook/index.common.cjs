...
var assert$3 = require('@alova/shared/assert');
...
const checkMethodRole = ({ meta }, metaMatches) => {
    if (_function.isPlainObject(meta)) {
        for (const key in meta) {
            if (Object.prototype.hasOwnProperty.call(meta, key)) {
                const matchedMetaItem = metaMatches[key];
                if (_function.instanceOf(matchedMetaItem, RegExp) ? matchedMetaItem.test(meta[key]) : meta[key] === matchedMetaItem) {
                    return vars.trueValue;
                }
            }
        }
    }
    return vars.falseValue;
};
const waitForTokenRefreshed = (method, waitingList) => _function.newInstance(vars.PromiseCls, resolve => {
    vars.pushItem(waitingList, {
        method,
        resolve
    });
});
const callHandlerIfMatchesMeta = (method, authorizationInterceptor, defaultMeta, response) => {
    if (checkMethodRole(method, (authorizationInterceptor === null || authorizationInterceptor === void 0 ? void 0 : authorizationInterceptor.metaMatches) || defaultMeta)) {
        const handler = _function.isFn(authorizationInterceptor)
            ? authorizationInterceptor
            : _function.isPlainObject(authorizationInterceptor) && _function.isFn(authorizationInterceptor.handler)
...
const refreshTokenIfExpired = async (method, waitingList, updateRefreshStatus, handlerParams, refreshToken, tokenRefreshing) => {
    ...
    if (isExpired) {
        try {
            ...
            updateRefreshStatus(vars.falseValue);
            vars.splice(waitingList, 0, vars.len(waitingList)); // 清空waitingList
        }
    }
};
const onResponded2Record = (onRespondedHandlers) => {
    let successHandler = vars.undefinedValue;
    let errorHandler = vars.undefinedValue;
    let onCompleteHandler = vars.undefinedValue;
    if (_function.isFn(onRespondedHandlers)) {
        successHandler = onRespondedHandlers;
    }
    else if (_function.isPlainObject(onRespondedHandlers)) {
        const { onSuccess, onError, onComplete } = onRespondedHandlers;
        successHandler = _function.isFn(onSuccess) ? onSuccess : successHandler;
        errorHandler = _function.isFn(onError) ? onError : errorHandler;
        onCompleteHandler = _function.isFn(onComplete) ? onComplete : onCompleteHandler;
    }
    return {
 ...
 */
const createClientTokenAuthentication = ({ visitorMeta, login, logout, refreshToken, assignToken = _function.noop }) => {
    let tokenRefreshing = vars.falseValue;
    const waitingList = [];
    ...
    const onResponseRefreshToken = originalResponded => {
        ...
        return {
            ...
            onSuccess: async (response, method) => {
                ...
                return (respondedRecord.onSuccess || _function.$self)(response, method);
            }
        };
    };
    return {
 ...
 * @returns token认证拦截器函数
 */
const createServerTokenAuthentication = ({ visitorMeta, login, logout, refreshTokenOnSuccess, refreshTokenOnError, assignToken = _function.noop }) => {
    let tokenRefreshing = vars.falseValue;
    const waitingList = [];
    const onAuthRequired = onBeforeRequest => async (method) => {
        const isVisitorRole = checkMethodRole(method, visitorMeta || defaultVisitorMeta);
    ...
    const onResponseRefreshToken = onRespondedHandlers => {
        ...
        return {
            ...
            onError: async (error, method) => {
                ...
                return (respondedRecord.onError || _function.noop)(error, method);
            }
        };
    };
    return {
...
const coreAssert = assert$3.createAssert('');
const requestHookAssert = assert$3.createAssert('useRequest');
const watcherHookAssert = assert$3.createAssert('useWatcher');
const fetcherHookAssert = assert$3.createAssert('useFetcher');
 ...
 */
const assertMethod = (assert, methodInstance) => assert(_function.instanceOf(methodInstance, alova.Method), 'expected a method instance.');
...
function useCallback(onCallbackChange = _function.noop) {
    let callbacks = [];
    const setCallback = (fn) => {
        ...
        // 返回取消注册函数
        return () => {
            callbacks = vars.filterItem(callbacks, e => e !== fn);
            onCallbackChange(callbacks);
        };
    };
    const triggerCallback = (...args) => {
        if (callbacks.length > 0) {
            return vars.forEach(callbacks, fn => fn(...args));
        }
    };
...
const debounce = (fn, delay) => {
    let timer = vars.nullValue;
    return function debounceFn(...args) {
        const bindFn = fn.bind(this, ...args);
        const delayMill = _function.isNumber(delay) ? delay : delay(...args);
        timer && vars.clearTimeoutTimer(timer);
        if (delayMill > 0) {
            timer = vars.setTimeoutFn(bindFn, delayMill);
        }
        else {
            bindFn();
        }
...
const getHandlerMethod = (methodHandler, args = []) => {
    const methodInstance = _function.isFn(methodHandler) ? methodHandler(...args) : methodHandler;
    assert$3.createAssert('scene')(_function.instanceOf(methodInstance, alova.Method), 'hook handler must be a method instance or a function that returns method instance');
...
const removeStateCache = (namespace, key) => {
    const cachedState = stateCache[namespace];
    if (cachedState) {
        vars.deleteAttr(cachedState, key);
    }
};
...
function useHookToSendRequest(hookInstance, methodHandler, sendCallingArgs = []) {
    ...
    let methodInstance = _function.getHandlerMethod(methodHandler, currentHookAssert, sendCallingArgs);
    ...
    const { force: forceRequest = vars.falseValue, middleware = defaultMiddleware } = useHookConfig;
    const alovaInstance = _function.getContext(methodInstance);
    const { id } = alovaInstance;
    // 如果是静默请求，则请求后直接调用onSuccess，不触发onError，然后也不会更新progress
    const methodKey = _function.getMethodInternalKey(methodInstance);
    const { abortLast = vars.trueValue } = useHookConfig;
    hookInstance.m = methodInstance;
    return (async () => {
        // 初始化状态数据，在拉取数据时不需要加载，因为拉取数据不需要返回data数据
        let removeStates = _function.noop;
        let saveStates = _function.noop;
        let isNextCalled = vars.falseValue;
        let responseHandlePromise = vars.promiseResolve(vars.undefinedValue);
        let offDownloadEvent = _function.noop;
        let offUploadEvent = _function.noop;
        ...
        let controlledLoading = vars.falseValue;
        if (!isFetcher) {
            // 将初始状态存入缓存以便后续更新
        ...
        const guardNext = guardNextConfig => {
            isNextCalled = vars.trueValue;
            const { force: guardNextForceRequest = forceRequest, method: guardNextReplacingMethod = methodInstance } = guardNextConfig || {};
            const forceRequestFinally = _function.sloughConfig(guardNextForceRequest, [
                _function.newInstance((AlovaEventBase), methodInstance, sendCallingArgs)
            ]);
            ...
            // 每次发送请求都需要保存最新的控制器
            vars.pushItem(hookInstance.sf, saveStates);
            vars.pushItem(hookInstance.rf, removeStates);
            ...
            fromCache = () => methodInstance.fromCache || vars.falseValue;
        ...
        const middlewareCompletePromise = isFetcher
            ? middleware({
                ...
                proxyStates: _function.omit(frontStates, 'data'),
                controlFetching(control = vars.trueValue) {
            ...
            : middleware({
                ...
                controlLoading(control = vars.trueValue) {
                    controlledLoading = control;
                }
            }, guardNext);
        let finallyResponse = vars.undefinedValue;
        ...
        try {
            ...
            const afterSuccess = (data) => {
                ...
                else if (hookInstance.c.updateState !== vars.falseValue) {
                ...
                if (toUpdateResponse()) {
                    errorState.v = vars.undefinedValue;
                    // loading状态受控时将不再更改为false
                    !controlledLoading && (loadingState.v = vars.falseValue);
                    hookInstance.em.emit(KEY_SUCCESS, _function.newInstance((AlovaSuccessEvent), baseEvent, data, fromCache()));
                    hookInstance.em.emit(KEY_COMPLETE, _function.newInstance((AlovaCompleteEvent), baseEvent, KEY_SUCCESS, data, fromCache(), vars.undefinedValue));
                }
                return data;
            };
            finallyResponse =
                ...
                middlewareReturnedData !== vars.undefinedValue
                    ...
                    : isNextCalled
                        ? // 当middlewareCompletePromise为resolve时有两种可能
                            ...
                            await vars.promiseThen(responseHandlePromise, afterSuccess, () => afterSuccess(vars.undefinedValue))
                        : // 如果isNextCalled未被调用，则不返回数据
                            vars.undefinedValue;
            // 未调用next函数时，更新loading为false
            !isNextCalled && !controlledLoading && (loadingState.v = vars.falseValue);
        }
        catch (error) {
            if (toUpdateResponse()) {
                // 控制在输出错误消息
                ...
                // loading状态受控时将不再更改为false
                !controlledLoading && (loadingState.v = vars.falseValue);
                hookInstance.em.emit(KEY_ERROR, _function.newInstance((AlovaErrorEvent), baseEvent, error));
                hookInstance.em.emit(KEY_COMPLETE, _function.newInstance((AlovaCompleteEvent), baseEvent, KEY_ERROR, vars.undefinedValue, fromCache(), error));
            }
            throw error;
        }
        // 响应后解绑下载和上传事件
 ...
 */
function createRequestState(hookType, methodHandler, useHookConfig, initialData, immediate = vars.falseValue, watchingStates, debounceDelay = 0) {
    var _a;
    ...
    const { middleware, __referingObj: referingObject = { trackedKeys: {}, bindError: vars.falseValue } } = useHookConfig;
    let initialLoading = middleware ? vars.falseValue : !!immediate;
    ...
    if (immediate && !middleware) {
        ...
        try {
            const methodInstance = _function.getHandlerMethod(methodHandler, coreHookAssert(hookType));
            const alovaInstance = _function.getContext(methodInstance);
            const l1CacheResult = alovaInstance.l1Cache.get(_function.buildNamespacedCacheKey(alovaInstance.id, _function.getMethodInternalKey(methodInstance)));
            let cachedResponse = vars.undefinedValue;
            // 只同步检查缓存，因此对异步的l1Cache适配器不生效
            // 建议在客户端不设置异步的l1Cache适配器
            if (l1CacheResult && !_function.instanceOf(l1CacheResult, vars.PromiseCls)) {
                const [data, expireTimestamp] = l1CacheResult;
                // 如果没有过期时间则表示数据永不过期，否则需要判断是否过期
                if (!expireTimestamp || expireTimestamp > _function.getTime()) {
                    cachedResponse = data;
                }
            }
            const forceRequestFinally = _function.sloughConfig((_a = useHookConfig.force) !== null && _a !== void 0 ? _a : vars.falseValue);
            initialLoading = !!forceRequestFinally || !cachedResponse;
        }
        catch (error) { }
    }
    const { create, effectRequest, ref, objectify, exposeProvider, transformState2Proxy } = _function.statesHookHelper(alova.promiseStatesHook(), referingObject);
    const progress = {
        total: 0,
        loaded: 0
    };
    ...
    const data = create((_function.isFn(initialData) ? initialData() : initialData), 'data');
    const loading = create(initialLoading, 'loading');
    const error = create(vars.undefinedValue, 'error');
    ...
    const hasWatchingStates = watchingStates !== vars.undefinedValue;
    // 初始化请求事件
    // 统一的发送请求函数
    ...
    // 捕获异常避免异常继续向外抛出
    const wrapEffectRequest = (ro = referingObject, handler) => vars.promiseCatch(handleRequest(handler), error => {
     ...
     */
    const debouncingSendHandler = ref(debounce((delay, ro, handler) => wrapEffectRequest(ro, handler), (changedIndex) => _function.isNumber(changedIndex) ? (vars.isArray(debounceDelay) ? debounceDelay[changedIndex] : debounceDelay) : 0));
    // 在服务端渲染时不发送请求
    if (!alova.globalConfigMap.ssr) {
        effectRequest({
            ...
            removeStates: () => vars.forEach(hookInstance.rf, fn => fn()),
            saveStates: states => vars.forEach(hookInstance.sf, fn => fn(states)),
            frontStates: { ...frontStates, ...managedStatesProxy },
            watchingStates,
            immediate: immediate !== null && immediate !== void 0 ? immediate : vars.trueValue
        });
    }
    return exposeProvider({
        ...
        onError(handler) {
            ...
            referingObject.bindError = vars.trueValue;
...
function useFetcher(config = {}) {
    const props = createRequestState(3 /* EnumHookType.USE_FETCHER */, _function.noop, config);
    const { send } = props;
    vars.deleteAttr(props, 'send');
    return _function.objAssign(props, {
        /**
         * Fetch data
...
function useRequest(handler, config = {}) {
    const { immediate = vars.trueValue, initialData } = config;
...
function useWatcher(handler, watchingStates, config = {}) {
    watcherHookAssert(watchingStates && vars.len(watchingStates) > 0, 'expected at least one watching state');
 ...
 */
const assertSerialHandlers = (hookName, serialHandlers) => assert$3.createAssert(hookName)(vars.isArray(serialHandlers) && vars.len(serialHandlers) > 0, 'please use an array to represent serial requests');
/**
 * 创建串行请求中间件
 * @param serialHandlers 串行请求method获取函数
...
const serialMiddleware = (serialHandlers, hookMiddleware, serialRequestMethods = []) => {
    ...
    return ((ctx, next) => {
        hookMiddleware === null || hookMiddleware === void 0 ? void 0 : hookMiddleware(ctx, () => vars.promiseResolve(vars.undefinedValue));
        ctx.controlLoading();
        const loadingState = ctx.proxyStates.loading;
        loadingState.v = vars.trueValue;
        let serialPromise = next();
        for (const handler of serialHandlers) {
            serialPromise = vars.promiseThen(serialPromise, value => {
                const methodItem = handler(value, ...ctx.args);
                vars.pushItem(serialRequestMethods, methodItem);
                return methodItem.send();
            });
        }
        return serialPromise.finally(() => {
            loadingState.v = vars.falseValue;
        });
    });
};
...
var useSerialRequest = (serialHandlers, config = {}) => {
    ...
    const { ref, __referingObj } = _function.statesHookHelper(alova.promiseStatesHook());
    ...
    exposures.onError = decorateEvent(exposures.onError, (handler, event) => {
        event.method = methods[vars.len(methods) - 1];
        handler(event);
    });
    return exposures;
};
...
const setQueueRequestWaitSetting = (requestWaitSetting = 0) => {
    queueRequestWaitSetting = vars.isArray(requestWaitSetting)
        ? requestWaitSetting
        : [
            {
...
/** silentAssert */
const silentAssert = assert$3.createAssert('useSQRequest');
...
async function updateState(matcher, handleUpdate) {
    let updated = vars.falseValue;
    // 只处理符合条件的第一个Method实例，如果没有符合条件的实例，则不处理
    if (matcher) {
        const { update } = alova.promiseStatesHook();
        const methodKey = _function.getMethodInternalKey(matcher);
        const { id } = _function.getContext(matcher);
        const { s: frontStates, h: hookInstance } = getStateCache(id, methodKey);
        const updateStateCollection = _function.isFn(handleUpdate)
            ? { data: handleUpdate }
            : handleUpdate;
        let updatedDataColumnData = vars.undefinedValue;
        if (frontStates) {
            // 循环遍历更新数据，并赋值给受监管的状态
            vars.forEach(vars.objectKeys(updateStateCollection), stateName => {
                ...
                updatedData = vars.isArray(updatedData)
                    ? [...updatedData]
                    : _function.isObject(updatedData)
                        ? { ...updatedData }
                        : updatedData;
            ...
            updated = vars.trueValue;
        }
        // 如果更新了data，则需要同时更新缓存和持久化数据
        if (updatedDataColumnData !== vars.undefinedValue) {
...
var dateSerializer = {
    forward: data => (_function.instanceOf(data, Date) ? data.getTime() : vars.undefinedValue),
    backward: ts => _function.newInstance(Date, ts)
};

var regexpSerializer = {
    forward: data => (_function.instanceOf(data, RegExp) ? data.source : undefined),
    backward: source => _function.newInstance(RegExp, source)
};
...
const createSerializerPerformer = (customSerializers = {}) => {
    ...
    const serialize = (payload) => {
        if (_function.isObject(payload)) {
            payload = _function.walkObject(vars.isArray(payload) ? [...payload] : { ...payload }, value => {
                let finallyApplySerializerName = vars.undefinedValue;
                // 找到匹配的序列化器并进行值的序列化，未找到则返回原值
                const serializedValue = vars.objectKeys(serializers).reduce((currentValue, serializerName) => {
                    if (!finallyApplySerializerName) {
                        const serializedValueItem = serializers[serializerName].forward(currentValue);
                        if (serializedValueItem !== vars.undefinedValue) {
                ...
                const toStringTag = vars.ObjectCls.prototype.toString.call(value);
                if (toStringTag === '[object Object]') {
                    value = { ...value };
                }
                else if (vars.isArray(value)) {
                    value = [...value];
                }
                return finallyApplySerializerName !== vars.undefinedValue ? [finallyApplySerializerName, serializedValue] : value;
            });
        }
        return payload;
    };
     ...
     * 反序列化数据
     */
    const deserialize = (payload) => _function.isObject(payload)
        ? _function.walkObject(payload, value => {
            if (vars.isArray(value) && vars.len(value) === 2) {
...
const vDataCollectUnified = (target) => {
    ...
    vDataId && vDataIdCollectBasket && (vDataIdCollectBasket[vDataId] = vars.undefinedValue);
};
 ...
 * @returns 虚拟数据id或原数据
 */
const stringifyVData = (target, returnOriginalIfNotVData = vars.trueValue) => {
    vDataCollectUnified(target);
    const vDataIdRaw = target === null || target === void 0 ? void 0 : target[symbolVDataId];
    const vDataId = vDataIdRaw ? `[vd:${vDataIdRaw}]` : vars.undefinedValue;
    return vDataId || (returnOriginalIfNotVData ? target : vars.undefinedValue);
};
/**
 * 创建虚拟数据id收集的getter函数
...
Null.prototype = vars.ObjectCls.create(vars.nullValue, {
    [STR_VALUE_OF]: vars.valueObject(stringifyWithThis)
});
...
Undefined.prototype = vars.ObjectCls.create(vars.nullValue, {
    [STR_VALUE_OF]: vars.valueObject(stringifyWithThis)
});
 ...
 * @returns 虚拟响应数据代理实例
 */
var createVirtualResponse = (structure, vDataId = _function.uuid()) => {
    const transform2VData = (value, vDataIdInner = _function.uuid()) => {
        if (value === vars.nullValue) {
            value = _function.newInstance(Null);
        }
        else if (value === vars.undefinedValue) {
            value = _function.newInstance(Undefined);
        }
        else {
            const newValue = vars.ObjectCls(value);
            vars.defineProperty(newValue, STR_VALUE_OF, stringifyWithThis);
            vars.defineProperty(newValue, symbolOriginal, value);
            value = newValue;
        }
        vars.defineProperty(value, symbolVDataId, vDataIdInner);
        return value;
    };
    const virtualResponse = transform2VData(structure, vDataId);
    if (_function.isPlainObject(virtualResponse) || vars.isArray(virtualResponse)) {
        _function.walkObject(virtualResponse, value => transform2VData(value));
    }
 ...
 * @returns 具有原始类型的目标值
 */
const dehydrateVDataUnified = (target, deepDehydrate = vars.trueValue) => {
    const dehydrateItem = (value) => {
        vDataCollectUnified(value);
        if (value === null || value === void 0 ? void 0 : value[symbolVDataId]) {
            if (_function.instanceOf(value, Undefined)) {
                value = vars.undefinedValue;
            }
            else if (_function.instanceOf(value, Null)) {
                value = vars.nullValue;
            }
            else if (_function.instanceOf(value, Number) || _function.instanceOf(value, String) || _function.instanceOf(value, Boolean)) {
    ...
    if (deepDehydrate && (_function.isObject(newTarget) || vars.isArray(newTarget))) {
        _function.walkObject(newTarget, value => dehydrateItem(value));
    }
    return newTarget;
};
/**
...
let serializerPerformer = vars.undefinedValue;
...
const storageSetItem = async (key, payload) => {
    ...
    if (_function.isObject(payload)) {
        payload = _function.walkObject(vars.isArray(payload) ? [...payload] : { ...payload }, (value, key, parent) => {
            var _a;
            ...
            if (key === 'context' && ((_a = value === null || value === void 0 ? void 0 : value.constructor) === null || _a === void 0 ? void 0 : _a.name) === 'Alova') {
                return vars.undefinedValue;
            }
            const vDataId = value === null || value === void 0 ? void 0 : value[symbolVDataId];
            let primitiveValue = dehydrateVDataUnified(value, vars.falseValue);
            // 需要用原始值判断，否则像new Number(1)等包装类也会是[object Object]
            const toStringTag = _function.globalToString(primitiveValue);
            if (toStringTag === '[object Object]') {
                value = { ...value };
                primitiveValue = {};
            }
            ...
            if (vDataId) {
                ...
                if (_function.instanceOf(value, String)) {
                    for (let i = 0; i < vars.len(value); i += 1) {
...
const storageGetItem = async (key) => {
    ...
    serializerPerformer = serializerPerformer || createSerializerPerformer(customSerializers);
    return _function.isObject(storagedResponse)
        ? _function.walkObject(serializerPerformer.deserialize(storagedResponse), value => {
            // 将虚拟数据格式转换回虚拟数据实例
            if (_function.isObject(value) && (value === null || value === void 0 ? void 0 : value[vDataKey])) {
                const vDataId = value[vDataKey];
                const vDataValue = createVirtualResponse(value[vDataValueKey], vDataId);
                vars.forEach(vars.objectKeys(value), key => {
                    if (!vars.includes([vDataKey, vDataValueKey], key)) {
...
const push2PersistentSilentQueue = async (silentMethodInstance, queueName) => {
    ...
    vars.pushItem(currentQueue, silentMethodInstance.id);
...
const spliceStorageSilentMethod = async (queueName, targetSilentMethodId, newSilentMethod) => {
    ...
    if (index >= 0) {
        if (newSilentMethod) {
            vars.splice(currentQueue, index, 1, newSilentMethod.id);
            await persistSilentMethod(newSilentMethod);
        }
        else {
            vars.splice(currentQueue, index, 1);
        }
        await storageRemoveItem(silentMethodStorageKeyPrefix + targetSilentMethodId);
        // 队列为空时删除此队列
        vars.len(currentQueue) <= 0 && delete silentMethodIdQueueMap[queueName];
        if (vars.len(vars.objectKeys(silentMethodIdQueueMap)) > 0) {
            await storageSetItem(silentMethodIdQueueMapStorageKey, silentMethodIdQueueMap);
        }
        else {
            // 队列集合为空时移除它
...
const merge2SilentQueueMap = (queueMap) => {
    vars.forEach(vars.objectKeys(queueMap), targetQueueName => {
        const currentQueue = (silentQueueMap[targetQueueName] = silentQueueMap[targetQueueName] || []);
        vars.pushItem(currentQueue, ...queueMap[targetQueueName]);
    });
};
/**
...
const deepReplaceVData = (target, vDataResponse) => {
    ...
    const replaceVData = (value) => {
        ...
        if (_function.isString(value)) {
            return value.replace(_function.newInstance(vars.RegExpCls, regVDataId.source, 'g'), mat => mat in vDataResponse ? vDataResponse[mat] : mat);
        }
        return value;
    };
    if (_function.isObject(target) && !stringifyVData(target, vars.falseValue)) {
        _function.walkObject(target, replaceVData);
    }
    else {
 ...
 */
const updateQueueMethodEntities = (vDataResponse, targetQueue) => vars.PromiseCls.all(vars.mapItem(targetQueue, async (silentMethodItem) => {
...
const replaceVirtualResponseWithResponse = (virtualResponse, response) => {
    let vDataResponse = {};
    const vDataId = stringifyVData(virtualResponse, vars.falseValue);
    vDataId && (vDataResponse[vDataId] = response);
    if (_function.isObject(virtualResponse)) {
...
const bootSilentQueue = (queue, queueName) => {
    ...
    const emitWithRequestDelay = (queueName) => {
        ...
        if (nextSilentMethod) {
            const targetSetting = queueRequestWaitSetting.find(({ queue }) => _function.instanceOf(queue, vars.RegExpCls) ? vars.regexpTest(queue, queueName) : queue === queueName);
            // eslint-disable-next-line @typescript-eslint/no-use-before-define
            const callback = () => queue[0] && silentMethodRequest(queue[0]);
            const delay = (targetSetting === null || targetSetting === void 0 ? void 0 : targetSetting.wait) ? _function.sloughConfig(targetSetting.wait, [nextSilentMethod, queueName]) : 0;
            delay && delay > 0 ? vars.setTimeoutFn(callback, delay) : callback();
        }
    };
    /**
     * 运行单个silentMethod实例
    ...
    const silentMethodRequest = (silentMethodInstance, retryTimes = 0) => {
        ...
        setSilentMethodActive(silentMethodInstance, vars.trueValue);
        const { cache, id, behavior, entity, retryError = /.*/, maxRetryTimes = 0, backoff = { delay: defaultBackoffDelay }, resolveHandler = _function.noop, rejectHandler = _function.noop, emitter: methodEmitter, handlerArgs = [], virtualResponse, force } = silentMethodInstance;
        // 触发请求前事件
        globalSQEventManager.emit(BeforeEventKey, _function.newInstance((GlobalSQEvent), behavior, entity, silentMethodInstance, queueName, retryTimes));
        vars.promiseThen(entity.send(force), async (data) => {
            ...
            if (behavior === BEHAVIOR_SILENT) {
                ...
                // 此为延迟数据更新的实现
                if (_function.instanceOf(targetRefMethod, alova.Method) && updateStates && vars.len(updateStates) > 0) {
                    const updateStateCollection = {};
                    vars.forEach(updateStates, stateName => {
                ...
                // 触发全局的成功事件
                globalSQEventManager.emit(SuccessEventKey, _function.newInstance((GlobalSQSuccessEvent), behavior, entity, silentMethodInstance, queueName, retryTimes, data, vDataResponse));
            }
            // 设为非激活状态
            setSilentMethodActive(silentMethodInstance, vars.falseValue);
            ...
            if (behavior !== BEHAVIOR_SILENT) {
                ...
                // 每次请求错误都将触发错误回调
                const runGlobalErrorEvent = (retryDelay) => globalSQEventManager.emit(ErrorEventKey, _function.newInstance((GlobalSQErrorEvent), behavior, entity, silentMethodInstance, queueName, retryTimes, reason, retryDelay));
                ...
                if (_function.instanceOf(retryError, RegExp)) {
                    regRetryErrorMsg = retryError;
                }
                else if (_function.isObject(retryError)) {
                    regRetryErrorName = retryError.name;
                    regRetryErrorMsg = retryError.message;
                }
                const matchRetryError = (regRetryErrorName && vars.regexpTest(regRetryErrorName, errorName)) ||
                    (regRetryErrorMsg && vars.regexpTest(regRetryErrorMsg, errorMsg));
                // 如果还有重试次数则进行重试
                if (retryTimes < maxRetryTimes && matchRetryError) {
                    // 需要使用下次的retryTimes来计算延迟时间，因此这边需+1
                    const retryDelay = _function.delayWithBackoff(backoff, retryTimes + 1);
                    runGlobalErrorEvent(retryDelay);
                    vars.setTimeoutFn(() => {
                        retryTimes += 1;
                        silentMethodRequest(silentMethodInstance, retryTimes);
                        methodEmitter.emit('retry', _function.newInstance((ScopedSQRetryEvent), behavior, entity, silentMethodInstance, handlerArgs, retryTimes, retryDelay));
                    }, 
                    ...
                    methodEmitter.emit('fallback', _function.newInstance((ScopedSQErrorEvent), behavior, entity, silentMethodInstance, handlerArgs, reason));
                    globalSQEventManager.emit(FailEventKey$1, _function.newInstance((GlobalSQFailEvent), behavior, entity, silentMethodInstance, queueName, retryTimes, reason));
                }
            }
            // 设为非激活状态
            setSilentMethodActive(silentMethodInstance, vars.falseValue);
...
const pushNewSilentMethod2Queue = async (silentMethodInstance, cache, targetQueueName = DEFAULT_QUEUE_NAME, onBeforePush = () => []) => {
    ...
    const isNewQueue = vars.len(currentQueue) <= 0;
    const beforePushReturns = await Promise.all(onBeforePush());
    const isPush2Queue = !beforePushReturns.some(returns => returns === vars.falseValue);
    ...
    if (isPush2Queue) {
        ...
        vars.pushItem(currentQueue, silentMethodInstance);
        // 如果是新的队列且状态为已启动，则执行它
 ...
 */
class SilentMethod {
    constructor(entity, behavior, emitter, id = _function.uuid(), force, retryError, maxRetryTimes, backoff, resolveHandler, rejectHandler, handlerArgs, vDatas) {
        const thisObj = this;
    ...
    async replace(newSilentMethod) {
        ...
        if (queue) {
            vars.splice(queue, position, 1, newSilentMethod);
    ...
    setUpdateState(method, updateStateName = 'data') {
        if (method) {
            ...
            this.updateStates = vars.isArray(updateStateName) ? updateStateName : [updateStateName];
        }
    }
}
...
var convertPayload2SilentMethod = (payload) => {
    ...
    const deserializeMethod = (methodPayload) => {
        ...
        return _function.newInstance(alova.Method, type, dependentAlovaInstance, url, config, data);
    };
    const silentMethodInstance = _function.newInstance(SilentMethod, deserializeMethod(entity), behavior, createEventManager(), id, force, retryError, maxRetryTimes, backoff, resolveHandler, rejectHandler, handlerArgs);
    silentMethodInstance.cache = vars.trueValue;
    // targetRefMethod反序列化
    if (targetRefMethod) {
        silentMethodInstance.targetRefMethod = deserializeMethod(targetRefMethod);
    }
    // 将额外的内容放到silentMethod实例上
    vars.forEach(vars.objectKeys(payload), key => {
        if (!vars.includes([
            'id',
            'behavior',
            'emitter',
...
var loadSilentQueueMapFromStorage = async () => {
    ...
    vars.forEach(vars.objectKeys(silentMethodIdQueueMap), queueName => {
        const currentQueue = (silentQueueMap[queueName] = silentQueueMap[queueName] || []);
        vars.pushItem(readingPromises, ...vars.mapItem(silentMethodIdQueueMap[queueName], async (silentMethodId) => {
            ...
            serializedSilentMethodPayload &&
                vars.pushItem(currentQueue, convertPayload2SilentMethod(serializedSilentMethodPayload));
        }));
    });
    await vars.PromiseCls.all(readingPromises);
 ...
 */
const filterSilentMethods = async (methodNameMatcher, queueName = DEFAULT_QUEUE_NAME, filterActive = vars.falseValue) => {
    const matchSilentMethods = (targetQueue = []) => targetQueue.filter(silentMethodItem => {
        if (methodNameMatcher === vars.undefinedValue) {
            return vars.trueValue;
        }
        const name = _function.getConfig(silentMethodItem.entity).name || '';
        const retain = _function.instanceOf(methodNameMatcher, RegExp)
            ? vars.regexpTest(methodNameMatcher, name)
            : name === methodNameMatcher;
        return retain && (filterActive ? silentMethodItem.active : vars.trueValue);
    });
    return [
 ...
 */
const getSilentMethod = async (methodNameMatcher, queueName = DEFAULT_QUEUE_NAME, filterActive = vars.falseValue) => (await filterSilentMethods(methodNameMatcher, queueName, filterActive))[0];
 ...
 * @returns 是否为虚拟数据
 */
var isVData = (target) => !!stringifyVData(target, vars.falseValue) || vars.regexpTest(regVDataId, target);

/**
 * 全局的silentMethod实例，它将在第一个成功事件触发前到最后一个成功事件触发后有值（同步时段）
 * 通过此方式让onSuccess中的updateStateEffect内获得当前的silentMethod实例
 */
let currentSilentMethod = vars.undefinedValue;
/**
 * 创建SilentQueue中间件函数
 * @param config 配置对象
 * @returns 中间件函数
...
var createSilentQueueMiddlewares = (handler, config) => {
    ...
    let forceRequest = vars.falseValue;
    ...
    const createMethod = (...args) => {
        silentAssert(_function.isFn(handler), 'method handler must be a function. eg. useSQRequest(() => method)');
    ...
    const decorateRequestEvent = (requestExposure) => {
        ...
        requestExposure.onSuccess = decorateEvent(requestExposure.onSuccess, (handler, event) => {
            ...
            handler(_function.newInstance((ScopedSQSuccessEvent), behaviorFinally, event.method, silentMethodInstance, event.args, event.data));
        });
        requestExposure.onError = decorateEvent(requestExposure.onError, (handler, event) => {
            handler(_function.newInstance((ScopedSQErrorEvent), behaviorFinally, event.method, silentMethodInstance, event.args, event.error));
        });
        requestExposure.onComplete = decorateEvent(requestExposure.onComplete, (handler, event) => {
            handler(_function.newInstance((ScopedSQCompleteEvent), behaviorFinally, event.method, silentMethodInstance, event.args, event.status, event.data, event.error));
        });
    };
    /**
     * 中间件函数
    ...
    const middleware = ({ method, args, cachedResponse, proxyStates, config }, next) => {
        const { silentDefaultResponse, vDataCaptured, force = vars.falseValue } = config;
        // 因为behavior返回值可能会变化，因此每次请求都应该调用它重新获取返回值
        const baseEvent = AlovaEventBase.spawn(method, args);
        behaviorFinally = _function.sloughConfig(behavior, [baseEvent]);
        queueFinally = _function.sloughConfig(queue, [baseEvent]);
        forceRequest = _function.sloughConfig(force, [baseEvent]);
        // 置空临时收集变量
        // 返回前都需要置空它们
        const resetCollectBasket = () => {
            setVDataIdCollectBasket((handlerArgs = vars.undefinedValue));
        };
        // 如果设置了vDataCaptured，则先判断请求相关的数据是否包含虚拟数据
        if (_function.isFn(vDataCaptured)) {
            let hasVData = vDataIdCollectBasket && vars.len(vars.objectKeys(vDataIdCollectBasket)) > 0;
            if (!hasVData) {
                const { url, data } = method;
                const { params, headers } = _function.getConfig(method);
                _function.walkObject({ url, params, data, headers }, value => {
                    if (!hasVData && (stringifyVData(value, vars.falseValue) || vars.regexpTest(regVDataId, value))) {
                        hasVData = vars.trueValue;
                    }
                    return value;
                });
            }
            // 如果vDataCaptured有返回数据，则使用它作为响应数据，否则继续请求
            const customResponse = hasVData ? vDataCaptured(method) : vars.undefinedValue;
            if (customResponse !== vars.undefinedValue) {
                resetCollectBasket(); // 被vDataCaptured捕获时的重置
                return vars.promiseResolve(customResponse);
            }
        }
        if (behaviorFinally !== BEHAVIOR_STATIC) {
            // 等待队列中的method执行完毕
            const createSilentMethodPromise = () => {
                const queueResolvePromise = _function.newInstance(vars.PromiseCls, (resolveHandler, rejectHandler) => {
                    silentMethodInstance = _function.newInstance((SilentMethod), method, behaviorFinally, eventEmitter, vars.undefinedValue, !!forceRequest, retryError, maxRetryTimes, backoff, resolveHandler, rejectHandler, handlerArgs, vDataIdCollectBasket && vars.objectKeys(vDataIdCollectBasket));
                    resetCollectBasket(); // behavior为queue和silent时的重置
                });
                // onBeforePush和onPushed事件是同步绑定的，因此需要异步执行入队列才能正常触发事件
                vars.promiseThen(vars.promiseResolve(vars.undefinedValue), async () => {
                    const createPushEvent = () => _function.newInstance((ScopedSQEvent), behaviorFinally, method, silentMethodInstance, args);
                    ...
                    vars.len(eventEmitter.eventMap.fallback || []) <= 0 && behaviorFinally === BEHAVIOR_SILENT, queueFinally, 
            ...
            if (behaviorFinally === BEHAVIOR_QUEUE) {
                ...
                if (needSendRequest) {
                    // 手动设置为true
                    proxyStates.loading.v = vars.trueValue;
                }
                // 当使用缓存时，直接使用缓存，否则再进入请求队列
                return needSendRequest ? createSilentMethodPromise() : vars.promiseThen(vars.promiseResolve(cachedResponse));
            }
            const silentMethodPromise = createSilentMethodPromise();
            // 在silent模式下创建虚拟响应数据，虚拟响应数据可生成任意的虚拟数据
            const virtualResponse = (silentMethodInstance.virtualResponse = createVirtualResponse(_function.isFn(silentDefaultResponse) ? silentDefaultResponse() : vars.undefinedValue));
            vars.promiseThen(silentMethodPromise, realResponse => {
                // 获取到真实数据后更新过去
                proxyStates.data.v = realResponse;
            });
            // silent模式下，先立即返回虚拟响应值，然后当真实数据返回时再更新
            return vars.promiseResolve(virtualResponse);
        }
...
const updateStateEffect = async (matcher, handleUpdate) => {
    ...
    if (currentSilentMethod) {
        currentSilentMethod.setUpdateState(matcher, _function.isFn(updateState) ? vars.undefinedValue : vars.objectKeys(updateState));
...
const assert$2 = assert$3.createAssert('subscriber');
/**
 * 操作函数委托中间件
...
const actionDelegationMiddleware = (id) => {
    const { ref } = _function.statesHookHelper(alova.promiseStatesHook());
    const delegated = ref(vars.falseValue);
    return (context, next) => {
        ...
        if (!delegated.current) {
            ...
            delegated.current = vars.trueValue;
        }
        return next();
    };
};
...
const accessAction = (id, onMatch, silent = false) => {
    const matched = [];
    if (typeof id === 'symbol' || _function.isString(id) || _function.isNumber(id)) {
        actionsMap[id] && vars.pushItem(matched, ...actionsMap[id]);
    }
    else if (_function.instanceOf(id, RegExp)) {
        vars.forEach(vars.filterItem(vars.objectKeys(actionsMap), idItem => id.test(idItem)), idItem => {
            vars.pushItem(matched, ...actionsMap[idItem]);
        });
    }
...
var createSnapshotMethodsManager = (handler) => {
    ...
    return {
        ...
        save(methodInstance, force = vars.falseValue) {
            const key = _function.getMethodInternalKey(methodInstance);
            // 因为无法定位缓存中total数据的位置
            // 因此这边冗余维护这个字段
        ...
        get: (entityOrPage) => methodSnapshots[_function.getMethodInternalKey(_function.instanceOf(entityOrPage, (alova.Method)) ? entityOrPage : handler(entityOrPage))],
        remove(key) {
            if (key) {
...
const paginationAssert = assert$3.createAssert('usePagination');
const indexAssert = (index, rawData) => paginationAssert(_function.isNumber(index) && index < vars.len(rawData), 'index must be a number that less than list length');
var usePagination = (handler, config = {}) => {
    const { create, computed, ref, watch, exposeProvider, objectify, __referingObj: referingObject } = _function.statesHookHelper(alova.promiseStatesHook());
    const { preloadPreviousPage = vars.trueValue, preloadNextPage = vars.trueValue, total: totalGetter = res => res.total, data: dataGetter = res => res.data, append = vars.falseValue, initialPage = 1, initialPageSize = 10, watchingStates = [], initialData, immediate = vars.trueValue, middleware = _function.noop, force = _function.noop, ...others } = config;
    const handlerRef = ref(handler);
    const isReset = ref(vars.falseValue); // 用于控制是否重置
    ...
    const total = create(initialData ? totalGetter(initialData) : vars.undefinedValue, 'total');
    ...
    watch(watchingStates, () => {
        page.v = initialPage;
        isReset.current = vars.trueValue;
    });
    ...
    const pageCount = computed(() => {
        ...
        return totalVal !== vars.undefinedValue ? Math.ceil(totalVal / pageSize.v) : vars.undefinedValue;
    ...
    const states = useWatcher(getHandlerMethod, [...watchingStates, page.e, pageSize.e], {
        ...
        force: event => event.args[1] || (_function.isFn(force) ? force(event) : force),
        ...others
    });
    const { send } = states;
    ...
    const canPreload = async (payload) => {
        const { rawData = nestedData.v, preloadPage, fetchMethod, forceRequest = vars.falseValue, isNextPage = vars.falseValue } = payload;
        const { e: expireMilliseconds } = _function.getLocalCacheConfigParam(fetchMethod);
        // 如果缓存时间小于等于当前时间，表示没有设置缓存，此时不再预拉取数据
        // 或者已经有缓存了也不预拉取
        if (expireMilliseconds(vars.MEMORY) <= _function.getTime()) {
            return vars.falseValue;
        }
        if (forceRequest) {
            return vars.trueValue;
        ...
        const exceedPageCount = pageCountVal
            ...
            : isNextPage // 如果是判断预加载下一页数据且没有pageCount的情况下，通过最后一页数据量是否达到pageSize来判断
                ? vars.len(listDataGetter(rawData)) < pageSize.v
                : vars.falseValue;
        return preloadPage > 0 && !exceedPageCount;
    };
    // 预加载下一页数据
    const fetchNextPage = async (rawData, force = vars.falseValue) => {
        ...
        if (preloadNextPage &&
            (await canPreload({
                ...
                fetchMethod,
                isNextPage: vars.trueValue,
                forceRequest: force
            }))) {
            vars.promiseCatch(fetch(fetchMethod, force), _function.noop);
        }
    };
    // 预加载上一页数据
    const fetchPreviousPage = async (rawData) => {
        ...
        if (preloadPreviousPage &&
            ...
            vars.promiseCatch(fetch(fetchMethod), _function.noop);
        }
    };
    ...
    const isLastPage = computed(() => {
        ...
        const pageCountVal = pageCount.v;
        const dataLen = vars.isArray(statesDataVal) ? vars.len(statesDataVal) : 0;
        ...
        if (snapshotItem) {
            await alova.setCache(snapshotItem.entity, (rawData) => {
                ...
                if (rawData) {
                    ...
                    vars.splice(cachedListData, 0, vars.len(cachedListData), ...data.v);
                    return rawData;
                }
            });
        }
    ...
    onFetchSuccess(({ method, data: rawData }) => {
        ...
        const snapshotItem = getSnapshotMethods(page.v);
        if (snapshotItem && _function.getMethodInternalKey(snapshotItem.entity) === _function.getMethodInternalKey(method)) {
            // 如果追加数据，才更新data
            ...
            if (append) {
                ...
                const replaceNumber = vars.len(dataRaw) % pageSizeVal;
                if (replaceNumber > 0) {
                    const rawData = [...data.v];
                    vars.splice(rawData, (page.v - 1) * pageSizeVal, replaceNumber, ...listData);
                    data.v = rawData;
                }
            }
            else {
    ...
    states.onSuccess(({ data: rawData, args: [refreshPage, isRefresh], method }) => {
        ...
        total.v = cachedTotal !== vars.undefinedValue ? cachedTotal : totalGetter(typedRawData);
        if (!isRefresh) {
        ...
        paginationAssert(vars.isArray(listData), 'Got wrong array, did you return the correct array of list in `data` function');
        // 如果追加数据，才更新data
        if (append) {
            // 如果是reset则先清空数据
            if (isReset.current) {
            ...
            if (refreshPage === vars.undefinedValue) {
                data.v = [...data.v, ...listData];
            }
            else if (refreshPage) {
                const rawData = [...data.v];
                // 如果是刷新页面，则是替换那一页的数据
                vars.splice(rawData, (refreshPage - 1) * pageSizeVal, pageSizeVal, ...listData);
                data.v = rawData;
            }
        }
        else {
    ...
    // 请求成功与否，都要重置isReset
    states.onComplete(() => {
        isReset.current = vars.falseValue;
    });
    // 获取列表项所在位置
    ...
    const { addQueue: add2AsyncQueue, onComplete: onAsyncQueueRunComplete } = ref(_function.createAsyncQueue()).current;
    /**
    ...
    const refresh = (pageOrItemPage = page.v) => {
        ...
        if (append) {
            if (!_function.isNumber(pageOrItemPage)) {
            ...
            // 更新当前页数据
            vars.promiseCatch(send(refreshPage, vars.trueValue), _function.noop);
        }
        else {
            paginationAssert(_function.isNumber(refreshPage), 'unable to calculate refresh page by item in pagination mode');
            // 页数相等，则刷新当前页，否则fetch数据
            vars.promiseCatch(refreshPage === page.v
                ? send(vars.undefinedValue, vars.trueValue)
                : fetch(handler(refreshPage, pageSize.v), vars.trueValue), _function.noop);
        }
    };
    // 删除除此usehook当前页和下一页的所有相关缓存
    const invalidatePaginationCache = async (all = vars.falseValue) => {
        const pageVal = page.v;
        const snapshotObj = methodSnapshots();
        let snapshots = vars.objectValues(snapshotObj);
        if (all) {
            removeSnapshot();
        }
        else {
            // 筛选出上一页、当前页、下一页的数据
            const excludeSnapshotKeys = vars.mapItem(vars.filterItem([getSnapshotMethods(pageVal - 1), getSnapshotMethods(pageVal), getSnapshotMethods(pageVal + 1)], Boolean), ({ entity }) => _function.getMethodInternalKey(entity));
            snapshots = vars.mapItem(vars.filterItem(vars.objectKeys(snapshotObj), key => !vars.includes(excludeSnapshotKeys, key)), key => {
                const item = snapshotObj[key];
                delete snapshotObj[key];
                return item;
            });
        }
        await alova.invalidateCache(vars.mapItem(snapshots, ({ entity }) => entity));
    };
    // 单独拿出来的原因是
    ...
    const resetCache = async () => {
        ...
        if (snapshotItem) {
            ...
            fetchNextPage(vars.undefinedValue, vars.len(cachedListData) < pageSize.v);
        }
    };
    // 统一更新总条数
    const updateTotal = (offset) => {
        ...
        // 更新当前页
        const totalVal = total.v;
        if (_function.isNumber(totalVal)) {
            const offsetedTotal = Math.max(totalVal + offset, 0);
            total.v = offsetedTotal;
            const pageVal = page.v;
            // 更新冗余的total字段
            vars.forEach([getSnapshotMethods(pageVal - 1), getSnapshotMethods(pageVal), getSnapshotMethods(pageVal + 1)], item => {
    ...
    const insert = (item, position = 0) => {
        ...
        return add2AsyncQueue(async () => {
            const index = _function.isNumber(position) ? position : getItemIndex(position) + 1;
            let popItem = vars.undefinedValue;
            const rawData = [...data.v];
            // 当前展示的项数量刚好是pageSize的倍数时，才需要去掉一项数据，保证操作页的数量为pageSize
            if (vars.len(rawData) % pageSize.v === 0) {
                popItem = rawData.pop();
            }
            // 插入位置为空默认插到最前面
            vars.splice(rawData, index, 0, item);
            data.v = rawData;
            updateTotal(1);
            // 当前页的缓存同步更新
    ...
    const remove = (...positions) => {
        onAsyncQueueRunComplete(resetCache); // 执行结束需要重置缓存
        return add2AsyncQueue(async () => {
            const indexes = vars.mapItem(positions, position => {
                const index = _function.isNumber(position) ? position : getItemIndex(position);
            ...
            if (snapshotItem) {
                await alova.setCache(snapshotItem.entity, (rawData) => {
                    if (rawData) {
                        const cachedListData = listDataGetter(rawData);
                        // 从下一页列表的头部开始取补位数据
                        if (vars.isArray(cachedListData)) {
                            vars.pushItem(fillingItems, ...vars.splice(cachedListData, 0, vars.len(indexes)));
                        }
                        return rawData;
                    }
                });
            ...
            const fillingItemsLen = vars.len(fillingItems);
            if (fillingItemsLen > 0 || isLastPageVal) {
                // 删除指定索引的数据
                const newListData = vars.filterItem(data.v, (_, index) => !vars.includes(indexes, index));
                // 翻页模式下，如果是最后一页且全部项被删除了，则往前翻一页
                if (!append && isLastPageVal && vars.len(newListData) <= 0) {
                    page.v = pageVal - 1;
                }
                else if (fillingItemsLen > 0) {
                    vars.pushItem(newListData, ...fillingItems);
                }
                data.v = newListData;
            }
    ...
    const replace = (item, position) => add2AsyncQueue(async () => {
        paginationAssert(position !== vars.undefinedValue, 'expect specify the replace position');
        const index = _function.isNumber(position) ? position : getItemIndex(position);
        indexAssert(index, data.v);
        const rawData = [...data.v];
        vars.splice(rawData, index, 1, item);
        data.v = rawData;
        // 当前页的缓存同步更新
        await updateCurrentPageCache();
    });
     ...
     */
    const reload = () => {
        vars.promiseThen(invalidatePaginationCache(vars.trueValue), () => {
            isReset.current = vars.trueValue;
            page.v === initialPage ? vars.promiseCatch(send(), _function.noop) : (page.v = initialPage);
        });
    };
...
function useSQRequest(handler, config = {}) {
    const { exposeProvider, __referingObj: referingObj } = _function.statesHookHelper(alova.promiseStatesHook());
    const { middleware = _function.noop } = config;
...
const useAutoRequest = (handler, config = {}) => {
    let notifiable = vars.trueValue;
    const { enableFocus = vars.trueValue, enableVisibility = vars.trueValue, enableNetwork = vars.trueValue, pollingTime = 0, throttle = 1000 } = config;
    const { onMounted, onUnmounted, __referingObj: referingObject } = _function.statesHookHelper(alova.promiseStatesHook());
    ...
    const notify = () => {
        if (notifiable) {
            states.send();
            if (throttle > 0) {
                notifiable = vars.falseValue;
                setTimeout(() => {
                    notifiable = vars.trueValue;
                }, throttle);
            }
        }
    };
    let offNetwork = _function.noop;
    let offFocus = _function.noop;
    let offVisiblity = _function.noop;
    let offPolling = _function.noop;
    onMounted(() => {
        if (!alova.globalConfigMap.ssr) {
...
const captchaAssert = assert$3.createAssert(hookPrefix$1);
var useCaptcha = (handler, config = {}) => {
    const { initialCountdown, middleware } = config;
    captchaAssert(initialCountdown === vars.undefinedValue || initialCountdown > 0, 'initialCountdown must be greater than 0');
    const { create, ref, objectify, exposeProvider, __referingObj: referingObject } = _function.statesHookHelper(alova.promiseStatesHook());
    const countdown = create(0, 'countdown');
    const requestReturned = useRequest(handler, {
        ...config,
        __referingObj: referingObject,
        immediate: vars.falseValue,
        managedStates: objectify([countdown], 's'),
        // eslint-disable-next-line @typescript-eslint/no-use-before-define
        middleware: middleware ? (ctx, next) => middleware({ ...ctx, send }, next) : vars.undefinedValue
    });
    const timer = ref(vars.undefinedValue);
    const send = (...args) => _function.newInstance(vars.PromiseCls, (resolve, reject) => {
        if (countdown.v <= 0) {
            requestReturned
                .send(...args)
                .then(result => {
            ...
            reject(_function.newInstance(assert$3.AlovaError, hookPrefix$1, 'the countdown is not over yet'));
        }
    });
    return exposeProvider({
        ...requestReturned,
...
const getStoragedKey = (methodInstance, id) => `alova/form-${id || _function.getMethodInternalKey(methodInstance)}`;
const sharedStates = {};
const cloneFormData = (form) => {
    const shallowClone = (value) => (vars.isArray(value) ? [...value] : _function.isPlainObject(value) ? { ...value } : value);
    return _function.walkObject(shallowClone(form), shallowClone);
};
var useForm = (handler, config = {}) => {
    const typedSharedStates = sharedStates;
    const { id, initialForm, store, resetAfterSubmiting, immediate = vars.falseValue, middleware } = config;
    alova.promiseStatesHook();
    const { create: $, ref: useFlag$, onMounted: onMounted$, watch: watch$, objectify, exposeProvider, __referingObj: referingObject } = _function.statesHookHelper(alova.promiseStatesHook());
    const isStoreObject = _function.isPlainObject(store);
    ...
    const sharedState = id ? typedSharedStates[id] : vars.undefinedValue;
    const form = $(cloneFormData(initialForm), 'form');
    const methodHandler = handler;
    const eventManager = createEventManager();
    // 使用计算属性，避免每次执行此use hook都调用一遍methodHandler
    const initialMethodInstance = useFlag$(_function.sloughConfig(methodHandler, [form.v]));
    const storageContext = _function.getContext(initialMethodInstance.current).l2Cache;
    const storagedKey = getStoragedKey(initialMethodInstance.current, id);
    const reseting = useFlag$(vars.falseValue);
    const serializerPerformer = useFlag$(createSerializerPerformer(isStoreObject ? store.serializers : vars.undefinedValue));
    ...
    const originalHookProvider = useRequest((...args) => methodHandler(form.v, ...args), {
        ...
        immediate: enableStore || sharedState ? vars.falseValue : immediate
    });
    /**
     * 重置form数据
     */
    const reset = () => {
        reseting.current = vars.trueValue;
    ...
    if (id) {
        ...
        if (!sharedState) {
            isCreateShardState.current = vars.trueValue;
        }
        // 只保存创建hook的共享状态
    ...
    onMounted$(() => {
        ...
        if (enableStore && !sharedState) {
            ...
            if (storagedForm) {
                form.v = storagedForm;
                // 触发持久化数据恢复事件
                eventManager.emit(RestoreEventKey, vars.undefinedValue);
    ...
    watch$([form], () => {
        if (reseting.current || !enableStore) {
            reseting.current = vars.falseValue;
            return;
        }
...
const assert$1 = assert$3.createAssert(hookPrefix);
var useRetriableRequest = (handler, config = {}) => {
    const { retry = 3, backoff = { delay: 1000 }, middleware = _function.noop } = config;
    const { ref: useFlag$, exposeProvider, __referingObj: referingObject } = _function.statesHookHelper(alova.promiseStatesHook());
    const eventManager = createEventManager();
    const retryTimes = useFlag$(0);
    const stopManuallyError = useFlag$(vars.undefinedValue); // 停止错误对象，在手动触发停止时有值
    const methodInstanceLastest = useFlag$(vars.undefinedValue);
    const argsLatest = useFlag$(vars.undefinedValue);
    const currentLoadingState = useFlag$(vars.falseValue);
    const requesting = useFlag$(vars.falseValue); // 是否正在请求
    const retryTimer = useFlag$(vars.undefinedValue);
    const promiseObj = useFlag$(_function.usePromise());
    const requestResolved = useFlag$(vars.falseValue);
    const emitOnFail = (method, args, error) => {
        if (requestResolved.current) {
            return;
        }
        requestResolved.current = vars.trueValue;
        // 需要异步触发onFail，让onError和onComplete先触发
        vars.setTimeoutFn(() => {
            eventManager.emit(FailEventKey, _function.newInstance((RetriableFailEvent), AlovaEventBase.spawn(method, args), error, retryTimes.current));
            stopManuallyError.current = vars.undefinedValue;
    ...
    const nestedHookProvider = useRequest(handler, {
        ...
        middleware(ctx, next) {
            ...
            }, () => vars.promiseResolve(vars.undefinedValue));
            const { proxyStates, args, send, method, controlLoading } = ctx;
            const setLoading = (loading = vars.falseValue) => {
             ...
             */
            return vars.promiseThen(Promise.race([next(), promiseObj.current.promise]), 
            // 请求成功时设置loading为false
            val => {
                // retryTimes.current = 0; // 重置已重试次数
                requesting.current = vars.falseValue;
                setLoading();
                return val;
            }, 
            // 请求失败时触发重试机制
            error => {
                ...
                if (!stopManuallyError.current && (_function.isNumber(retry) ? retryTimes.current < retry : retry(error, ...args))) {
                    retryTimes.current += 1;
                    // 计算重试延迟时间
                    const retryDelay = _function.delayWithBackoff(backoff, retryTimes.current);
                    // 延迟对应时间重试
                    retryTimer.current = vars.setTimeoutFn(() => {
                        // 如果手动停止了则不再触发重试
                        vars.promiseCatch(send(...args), _function.noop); // 捕获错误不再往外抛，否则重试时也会抛出错误
                        // 触发重试事件
                        eventManager.emit(RetryEventKey, _function.newInstance((RetriableRetryEvent), AlovaEventBase.spawn(method, args), retryTimes.current, retryDelay));
                    }, retryDelay);
                }
                else {
                    setLoading();
                ...
                requesting.current = vars.falseValue;
                // 返回reject执行后续的错误流程
                return vars.promiseReject(error);
            });
        }
    });
    /**
    ...
    const stop = () => {
        ...
        stopManuallyError.current = _function.newInstance(assert$3.AlovaError, hookPrefix, 'stop retry manually');
        if (requesting.current) {
            ...
            setTimeout(() => {
                promiseObj.current = _function.usePromise();
            });
            nestedHookProvider.update({ error: stopManuallyError.current, loading: vars.falseValue });
            currentLoadingState.current = vars.falseValue;
...
const buildCompletedURL = (baseURL, url, params) => {
    ...
    // 将params对象转换为get字符串
    // 过滤掉值为undefined的
    const paramsStr = vars.mapItem(vars.filterItem(vars.objectKeys(params), key => params[key] !== vars.undefinedValue), key => `${key}=${params[key]}`).join('&');
...
const assert = assert$3.createAssert('useSSE');
const MessageType = {
    Open: 'open',
    Error: 'error',
    Message: 'message'
...
var useSSE = (handler, config = {}) => {
    const { initialData, withCredentials, interceptByGlobalResponded = vars.trueValue, 
    /** abortLast = trueValue, */
    immediate = vars.falseValue } = config;
    // ! 暂时不支持指定 abortLast
    const abortLast = vars.trueValue;
    let { memorize } = alova.promiseStatesHook();
    memorize !== null && memorize !== void 0 ? memorize : (memorize = _function.$self);
    const { create, ref, onMounted, onUnmounted, objectify, exposeProvider } = _function.statesHookHelper(alova.promiseStatesHook());
    const usingArgs = ref([]);
    const eventSource = ref(vars.undefinedValue);
    const sendPromiseObject = ref(vars.undefinedValue);
    ...
    const responseSuccessHandler = ref(_function.$self);
    const responseErrorHandler = ref(throwFn);
    const responseCompleteHandler = ref(_function.noop);
    /**
     * 设置响应拦截器，在每次 send 之后都需要调用
     */
    const setResponseHandler = (instance) => {
        // responsed 从 3.0 开始移除
        const { responded } = _function.getOptions(instance);
        responseUnified = responded;
        if (_function.isFn(responseUnified)) {
            responseSuccessHandler.current = responseUnified;
        }
        else if (responseUnified && _function.isPlainObject(responseUnified)) {
            const { onSuccess: successHandler, onError: errorHandler, onComplete: completeHandler } = responseUnified;
            responseSuccessHandler.current = _function.isFn(successHandler) ? successHandler : responseSuccessHandler.current;
            responseErrorHandler.current = _function.isFn(errorHandler) ? errorHandler : responseErrorHandler.current;
            responseCompleteHandler.current = _function.isFn(completeHandler) ? completeHandler : responseCompleteHandler.current;
        }
    };
    /**
     * 处理响应任务，失败时不缓存数据
    ...
    const handleResponseTask = async (handlerReturns) => {
        const { headers, transform: transformFn = _function.$self } = _function.getConfig(methodInstance);
    ...
    const createSSEEvent = async (eventFrom, dataOrError) => {
        ...
        const globalSuccess = interceptByGlobalResponded ? responseSuccessHandler.current : _function.$self;
        const globalError = interceptByGlobalResponded ? responseErrorHandler.current : throwFn;
        const globalFinally = interceptByGlobalResponded ? responseCompleteHandler.current : _function.noop;
        const p = vars.promiseFinally(vars.promiseThen(dataOrError, res => handleResponseTask(globalSuccess(res, methodInstance)), error => handleResponseTask(globalError(error, methodInstance))), 
        // finally
        () => {
            globalFinally(methodInstance);
        });
        // 无论如何，函数返回的 Promise 对象一定都会 fulfilled
        return vars.promiseThen(p, 
    ...
    const sendSSEEvent = (callback) => (event) => {
        if (event.error === vars.undefinedValue) {
    ...
    const onCustomEvent = (eventName, callbackHandler) => {
        ...
        if (!currentMap.has(eventName)) {
            ...
            (_a = eventSource.current) === null || _a === void 0 ? void 0 : _a.addEventListener(eventName, event => {
                vars.promiseThen(createSSEEvent(eventName, Promise.resolve(event.data)), sendSSEEvent(trigger));
            });
        }
    ...
    const esOpen = memorize(() => {
        ...
        vars.promiseThen(createSSEEvent(MessageType.Open, Promise.resolve()), event => eventManager.emit(SSEOpenEventKey, event));
    ...
    const esError = memorize((event) => {
        ...
        vars.promiseThen(createSSEEvent(MessageType.Error, Promise.reject((_a = event === null || event === void 0 ? void 0 : event.message) !== null && _a !== void 0 ? _a : 'SSE Error')), sendSSEEvent(event => eventManager.emit(SSEMessageEventKey, event)));
        (_b = sendPromiseObject.current) === null || _b === void 0 ? void 0 : _b.resolve();
    });
    const esMessage = memorize((event) => {
        vars.promiseThen(createSSEEvent(MessageType.Message, Promise.resolve(event.data)), sendSSEEvent(event => eventManager.emit(SSEMessageEventKey, event)));
    });
    /**
    ...
    const connect = (...args) => {
        ...
        if (!promiseObj) {
            promiseObj = sendPromiseObject.current = _function.usePromise();
            // open 后清除 promise 对象
            promiseObj &&
                promiseObj.promise.finally(() => {
                    promiseObj = vars.undefinedValue;
                });
        }
        ...
        const { params } = _function.getConfig(methodInstance);
        ...
        customEventMap.current.forEach(([_, eventTrigger], eventName) => {
            es === null || es === void 0 ? void 0 : es.addEventListener(eventName, event => {
                vars.promiseThen(createSSEEvent(eventName, Promise.resolve(event.data)), sendSSEEvent(eventTrigger));
            });
        });
