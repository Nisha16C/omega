...
var assert$1 = require('@alova/shared/assert');
...
const assert = assert$1.createAssert('RateLimit');
...
class LimitedMethod extends HookedMethod {
    constructor(method, limiterKey, limiter) {
        ...
        this.keyGetter = _function.isFn(limiterKey) ? () => limiterKey(method) : () => limiterKey;
    }
    getLimiterKey() {
...
function createRateLimiter(options = {}) {
    ...
    const limitedMethodWrapper = createServerHook((method, handlerOptions = {}) => {
        var _a;
        const { key = _function.uuid() } = handlerOptions;
        const storage = (_a = options.storage) !== null && _a !== void 0 ? _a : _function.getOptions(method).l2Cache;
    ...
    return new HookedMethod(method, forceRequest => new Promise((resolve, reject) => {
        const sendRequest = () => {
            method
                ...
                if (_function.isNumber(retry) ? retryTimes < retry : retry(error)) {
                    retryTimes += 1;
                    // calculate retry delay time with pram `backoff` and current retry times.
                    const retryDelay = _function.delayWithBackoff(backoff, retryTimes);
