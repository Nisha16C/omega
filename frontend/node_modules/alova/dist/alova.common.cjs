...
var assert = require('@alova/shared/assert');

exports.globalConfigMap = {
    autoHitCache: 'global',
    ssr: vars.defaultIsSSR
};
/**
...
var defaultCacheLogger = (response, methodInstance, cacheMode, tag) => {
    ...
    const isRestoreMode = cacheMode === vars.STORAGE_RESTORE;
    const hdStyle = '\x1B[42m%s\x1B[49m';
    const labelStyle = '\x1B[32m%s\x1B[39m';
    const startSep = ` [HitCache]${url} `;
    const endSepFn = () => Array(vars.len(startSep) + 1).join('^');
...
const setWithCacheAdapter = async (namespace, key, data, expireTimestamp, cacheAdapter, hitSource, tag) => {
    ...
    if (expireTimestamp > _function.getTime() && data) {
        const methodCacheKey = _function.buildNamespacedCacheKey(namespace, key);
        await cacheAdapter.set(methodCacheKey, vars.filterItem([data, expireTimestamp === Infinity ? vars.undefinedValue : expireTimestamp, tag], Boolean));
        ...
        */
        if (hitSource) {
            ...
            vars.forEach(hitSource, sourceItem => {
                const isRegexp = _function.instanceOf(sourceItem, vars.RegExpCls);
                ...
                if (targetHitSourceKey) {
                    if (isRegexp && !hitSourceKeys[targetHitSourceKey]) {
                        vars.pushItem(hitSourceRegexpSources, targetHitSourceKey);
                    }
            ...
            const promises = vars.mapItem(vars.objectKeys(hitSourceKeys), async (hitSourceKey) => {
...
const removeWithCacheAdapter = async (namespace, key, cacheAdapter) => {
    const methodStoreKey = _function.buildNamespacedCacheKey(namespace, key);
...
const getRawWithCacheAdapter = async (namespace, key, cacheAdapter, tag) => {
    const storagedData = await cacheAdapter.get(_function.buildNamespacedCacheKey(namespace, key));
    if (storagedData) {
        // eslint-disable-next-line
        const [_, expireTimestamp, storedTag] = storagedData;
        // 如果没有过期时间则表示数据永不过期，否则需要判断是否过期
        if (storedTag === tag && (!expireTimestamp || expireTimestamp > _function.getTime())) {
            return storagedData;
        }
        // 如果过期，则删除缓存
...
const getWithCacheAdapter = async (namespace, key, cacheAdapter, tag) => {
    ...
    return rawData ? rawData[0] : vars.undefinedValue;
};
/**
 * clear all cached data
 */
const clearWithCacheAdapter = async (cacheAdapters) => vars.PromiseCls.all(cacheAdapters.map(cacheAdapter => cacheAdapter.clear()));
/**
...
const hitTargetCacheWithCacheAdapter = async (sourceKey, sourceName, cacheAdapter) => {
    ...
    if (sourceName) {
        ...
        if (unifiedHitSourceRegexpChannel && vars.len(unifiedHitSourceRegexpChannel)) {
            vars.forEach(unifiedHitSourceRegexpChannel, regexpStr => {
                const [source, flag] = regexpStr.split(regexpSourceFlagSeparator);
                if (_function.newInstance(vars.RegExpCls, source, flag).test(sourceNameStr)) {
                    vars.pushItem(matchedRegexpStrings, regexpStr);
                }
            });
            // parallel get hit key by matched regexps.
            await vars.PromiseCls.all(vars.mapItem(matchedRegexpStrings, async (regexpString) => {
    ...
    const removeWithTargetKey = async (targetKey) => {
        try {
            ...
            for (const sourceKey in sourceTargetKeyMap) {
                ...
                if (targetKeys) {
                    vars.deleteAttr(targetKeys, targetKey);
                }
            }
        }
        catch (error) {
    ...
    const accessedKeys = {};
    await vars.PromiseCls.all(vars.mapItem(vars.objectKeys(sourceTargetKeyMap), async (sourceKey) => {
        const targetKeys = sourceTargetKeyMap[sourceKey];
        if (targetKeys) {
            ...
            for (const key in targetKeys) {
                if (!accessedKeys[key]) {
                    ...
                    vars.pushItem(removingPromises, removeWithTargetKey(key));
                }
            }
            await vars.PromiseCls.all(removingPromises);
        }
    }));
    // update source key if there is still has keys.
    // remove source key if its keys is empty.
    const unifiedHitSourceRegexpChannelLen = vars.len(unifiedHitSourceRegexpChannel || []);
    await vars.PromiseCls.all(vars.mapItem(vars.objectKeys(sourceTargetKeyMap), async (sourceKey) => {
        const targetKeys = sourceTargetKeyMap[sourceKey];
        if (targetKeys) {
            if (vars.len(vars.objectKeys(targetKeys))) {
                ...
                if (sourceKey.includes(hitSourceRegexpPrefix) && unifiedHitSourceRegexpChannel) {
                    unifiedHitSourceRegexpChannel = vars.filterItem(unifiedHitSourceRegexpChannel, rawRegexpStr => hitSourceRegexpCacheKey(rawRegexpStr) !== sourceKey);
                }
            }
        }
    }));
    // update unified hit source regexp channel if its length was changed.
    if (unifiedHitSourceRegexpChannelLen !== vars.len(unifiedHitSourceRegexpChannel || [])) {
...
var cloneMethod = (methodInstance) => {
    ...
    const ctx = _function.getContext(methodInstance);
    newConfig.headers = { ...headers };
    newConfig.params = { ...params };
    const newMethod = _function.newInstance((Method), methodInstance.type, ctx, methodInstance.url, newConfig, data);
    return _function.objAssign(newMethod, {
        ...methodInstance,
        config: newConfig
    });
};
...
const queryCache = async (matcher, { policy = 'all' } = {}) => {
    ...
    if (matcher && matcher.key) {
        const { id, l1Cache, l2Cache } = _function.getContext(matcher);
        const methodKey = _function.getMethodInternalKey(matcher);
        const { f: cacheFor, c: controlled, s: store, e: expireMilliseconds, t: tag } = _function.getLocalCacheConfigParam(matcher);
        // if it's controlled cache, it will return the result of cacheFor function.
        if (controlled) {
            return cacheFor();
        }
        let cachedData = policy !== 'l2' ? await getWithCacheAdapter(id, methodKey, l1Cache) : vars.undefinedValue;
        if (policy === 'l2') {
            cachedData = await getWithCacheAdapter(id, methodKey, l2Cache, tag);
        }
        else if (policy === 'all' && !cachedData) {
            if (store && expireMilliseconds(vars.STORAGE_RESTORE) > _function.getTime()) {
                cachedData = await getWithCacheAdapter(id, methodKey, l2Cache, tag);
            }
        }
        return cachedData;
...
const setCache = async (matcher, dataOrUpdater, { policy = 'all' } = {}) => {
    const methodInstances = vars.isArray(matcher) ? matcher : [matcher];
    const batchPromises = methodInstances.map(async (methodInstance) => {
        const { hitSource } = methodInstance;
        const { id, l1Cache, l2Cache } = _function.getContext(methodInstance);
        const methodKey = _function.getMethodInternalKey(methodInstance);
        const { e: expireMilliseconds, s: toStore, t: tag, c: controlled } = _function.getLocalCacheConfigParam(methodInstance);
        // don't set cache when it's controlled cache.
        if (controlled) {
            return;
        }
        let data = dataOrUpdater;
        if (_function.isFn(dataOrUpdater)) {
            let cachedData = policy !== 'l2' ? await getWithCacheAdapter(id, methodKey, l1Cache) : vars.undefinedValue;
            if (policy === 'l2' ||
                (policy === 'all' && !cachedData && toStore && expireMilliseconds(vars.STORAGE_RESTORE) > _function.getTime())) {
                cachedData = await getWithCacheAdapter(id, methodKey, l2Cache, tag);
            }
            data = dataOrUpdater(cachedData);
            if (data === vars.undefinedValue) {
                return;
            }
        }
        return vars.PromiseCls.all([
            policy !== 'l2' && setWithCacheAdapter(id, methodKey, data, expireMilliseconds(vars.MEMORY), l1Cache, hitSource),
            policy === 'l2' || (policy === 'all' && toStore)
                ? setWithCacheAdapter(id, methodKey, data, expireMilliseconds(vars.STORAGE_RESTORE), l2Cache, hitSource, tag)
                : vars.undefinedValue
        ]);
    });
    return vars.PromiseCls.all(batchPromises);
};
/**
 * 失效缓存
 * @param matcher Method实例匹配器
...
const invalidateCache = async (matcher) => {
    if (!matcher) {
        await vars.PromiseCls.all([clearWithCacheAdapter(usingL1CacheAdapters), clearWithCacheAdapter(usingL2CacheAdapters)]);
        return;
    }
    const methodInstances = vars.isArray(matcher) ? matcher : [matcher];
    const batchPromises = methodInstances.map(methodInstance => {
        const { id, l1Cache, l2Cache } = _function.getContext(methodInstance);
        const { c: controlled } = _function.getLocalCacheConfigParam(methodInstance);
    ...
    await vars.PromiseCls.all(batchPromises);
};
/**
...
const hitCacheBySource = async (sourceMethod) => {
    ...
    const { l1Cache, l2Cache } = _function.getContext(sourceMethod);
    const sourceKey = _function.getMethodInternalKey(sourceMethod);
    const { name: sourceName } = _function.getConfig(sourceMethod);
    const cacheAdaptersInvolved = {
        global: [...usingL1CacheAdapters, ...usingL2CacheAdapters],
        self: [l1Cache, l2Cache],
        close: []
    }[autoHitCache];
    if (cacheAdaptersInvolved && vars.len(cacheAdaptersInvolved)) {
        await vars.PromiseCls.all(vars.mapItem(cacheAdaptersInvolved, involvedCacheAdapter => hitTargetCacheWithCacheAdapter(sourceKey, sourceName, involvedCacheAdapter)));
    }
};
...
function sendRequest(methodInstance, forceRequest) {
    let fromCache = vars.trueValue;
    let requestAdapterCtrlsPromiseResolveFn;
    const requestAdapterCtrlsPromise = _function.newInstance(vars.PromiseCls, resolve => {
        requestAdapterCtrlsPromiseResolveFn = resolve;
    });
    const response = async () => {
        const { beforeRequest = _function.noop, responded, requestAdapter, cacheLogger } = _function.getOptions(methodInstance);
        const methodKey = _function.getMethodInternalKey(methodInstance);
        const { s: toStorage, t: tag, m: cacheMode, e: expireMilliseconds } = _function.getLocalCacheConfigParam(methodInstance);
        const { id, l1Cache, l2Cache, snapshots } = _function.getContext(methodInstance);
        // 获取受控缓存或非受控缓存
        const { cacheFor } = _function.getConfig(methodInstance);
        const { hitSource: methodHitSource } = methodInstance;
        // 如果当前method设置了受控缓存，则看是否有自定义的数据
        let cachedResponse = await (_function.isFn(cacheFor)
            ? cacheFor()
        ...
        if (cacheMode === vars.STORAGE_RESTORE && !cachedResponse) {
        ...
        const { params = {}, headers = {}, transform = _function.$self, shareRequest } = _function.getConfig(clonedMethod);
        ...
        let responseSuccessHandler = _function.$self;
        let responseErrorHandler = vars.undefinedValue;
        let responseCompleteHandler = _function.noop;
        // uniform handler of onSuccess, onError, onComplete
        if (_function.isFn(responded)) {
            responseSuccessHandler = responded;
        }
        else if (_function.isPlainObject(responded)) {
            const { onSuccess: successHandler, onError: errorHandler, onComplete: completeHandler } = responded;
            responseSuccessHandler = _function.isFn(successHandler) ? successHandler : responseSuccessHandler;
            responseErrorHandler = _function.isFn(errorHandler) ? errorHandler : responseErrorHandler;
            responseCompleteHandler = _function.isFn(completeHandler) ? completeHandler : responseCompleteHandler;
        }
        // 如果没有缓存则发起请求
        if (cachedResponse !== vars.undefinedValue) {
            requestAdapterCtrlsPromiseResolveFn(); // 遇到缓存将不传入ctrls
            // 打印缓存日志
            _function.sloughFunction(cacheLogger, defaultCacheLogger)(cachedResponse, clonedMethod, cacheMode, tag);
            responseCompleteHandler(clonedMethod);
            return cachedResponse;
        }
        fromCache = vars.falseValue;
         ...
         */
        const handleResponseTask = async (handlerReturns, responseHeaders, callInSuccess = vars.trueValue) => {
            ...
            const toCache = !requestBody || !_function.isSpecialRequestBody(requestBody);
            // 使用响应后最新的过期时间来缓存数据，避免因响应时间过长导致过期时间流失的问题
            if (toCache && callInSuccess) {
                try {
                    await vars.PromiseCls.all([
                        setWithCacheAdapter(id, methodKey, transformedData, expireMilliseconds(vars.MEMORY), l1Cache, methodHitSource),
                        toStorage &&
                            setWithCacheAdapter(id, methodKey, transformedData, expireMilliseconds(vars.STORAGE_RESTORE), l2Cache, methodHitSource, tag)
                    ]);
                }
                catch (error) { }
            }
        ...
        return vars.promiseFinally(vars.promiseThen(vars.PromiseCls.all([requestAdapterCtrls.response(), requestAdapterCtrls.headers()]), ([rawResponse, rawHeaders]) => {
            // 无论请求成功、失败，都需要首先移除共享的请求
            vars.deleteAttr(namespacedAdapterReturnMap, methodKey);
            return handleResponseTask(responseSuccessHandler(rawResponse, clonedMethod), rawHeaders);
        }, (error) => {
            // 无论请求成功、失败，都需要首先移除共享的请求
            vars.deleteAttr(namespacedAdapterReturnMap, methodKey);
            return _function.isFn(responseErrorHandler)
                ? // 响应错误时，如果未抛出错误也将会处理响应成功的流程，但不缓存数据
                    handleResponseTask(responseErrorHandler(error, clonedMethod), vars.undefinedValue, vars.falseValue)
                : vars.promiseReject(error);
        }), () => {
            responseCompleteHandler(clonedMethod);
        });
    };
    return {
        // 请求中断函数
        abort: () => {
            vars.promiseThen(requestAdapterCtrlsPromise, requestAdapterCtrls => requestAdapterCtrls && requestAdapterCtrls.abort());
        },
        onDownload: (handler) => {
            vars.promiseThen(requestAdapterCtrlsPromise, requestAdapterCtrls => requestAdapterCtrls && requestAdapterCtrls.onDownload && requestAdapterCtrls.onDownload(handler));
        },
        onUpload: (handler) => {
            vars.promiseThen(requestAdapterCtrlsPromise, requestAdapterCtrls => requestAdapterCtrls && requestAdapterCtrls.onUpload && requestAdapterCtrls.onUpload(handler));
        },
        response,
        fromCache: () => fromCache
    };
...
class Method {
    constructor(type, context, url, config, data) {
        this.dhs = [];
        this.uhs = [];
        this.fromCache = vars.undefinedValue;
        const abortRequest = () => {
            abortRequest.a();
        };
        abortRequest.a = _function.noop;
        const instance = this;
        const contextOptions = _function.getContextOptions(context);
        ...
        const globalLocalCache = _function.isPlainObject(contextOptions[mergedLocalCacheKey])
            ? contextOptions[mergedLocalCacheKey][type]
            : vars.undefinedValue;
        const hitSource = config && config.hitSource;
        // 合并参数
        vars.forEach(['timeout', 'shareRequest'], mergedKey => {
            if (contextOptions[mergedKey] !== vars.undefinedValue) {
                contextConcatConfig[mergedKey] = contextOptions[mergedKey];
            }
        });
        // 合并localCache
        if (globalLocalCache !== vars.undefinedValue) {
            contextConcatConfig[mergedLocalCacheKey] = globalLocalCache;
        }
        // 将hitSource统一处理成数组，且当有method实例时将它们转换为methodKey
        if (hitSource) {
            instance.hitSource = vars.mapItem(vars.isArray(hitSource) ? hitSource : [hitSource], sourceItem => _function.instanceOf(sourceItem, Method) ? _function.getMethodInternalKey(sourceItem) : sourceItem);
            vars.deleteAttr(config, 'hitSource');
        }
        instance.config = {
            ...contextConcatConfig,
            headers: {},
    ...
    onDownload(downloadHandler) {
        vars.pushItem(this.dhs, downloadHandler);
    ...
    onUpload(uploadHandler) {
        vars.pushItem(this.uhs, uploadHandler);
        return offEventCallback(uploadHandler, this.uhs);
    }
    /**
     * 通过method实例发送请求，返回promise对象
     */
    send(forceRequest = vars.falseValue) {
        const instance = this;
        const { response, onDownload, onUpload, abort, fromCache } = sendRequest(instance, forceRequest);
        vars.len(instance.dhs) > 0 &&
            onDownload((loaded, total) => vars.forEach(instance.dhs, handler => handler({ loaded, total })));
        vars.len(instance.uhs) > 0 && onUpload((loaded, total) => vars.forEach(instance.uhs, handler => handler({ loaded, total })));
        // 每次请求时将中断函数绑定给method实例，使用者也可通过methodInstance.abort()来中断当前请求
        instance.abort.a = abort;
        instance.fromCache = vars.undefinedValue;
        instance.promise = vars.promiseThen(response(), r => {
     ...
     */
    setName(name) {
        _function.getConfig(this).name = name;
    }
    generateKey() {
        return _function.key(this);
    }
    /**
    ...
    then(onfulfilled, onrejected) {
        return vars.promiseThen(this.send(), onfulfilled, onrejected);
    }
    /**
     ...
     */
        ...
        return vars.promiseCatch(this.send(), onrejected);
    }
    /**
     ...
     */
        ...
        return vars.promiseFinally(this.send(), onfinally);
    }
}
 ...
 * @param msg 断言消息
 */
const myAssert = assert.createAssert();
...
const localStorageAdapter = () => {
    ...
    const adapter = {
        set: (key, value) => {
            storage().setItem(key, vars.JSONStringify(value));
            l2CacheEmitter.emit(EVENT_SUCCESS_KEY, { type: 'set', key, value, container: storage() });
        },
        get: key => {
            const data = storage().getItem(key);
            const value = data ? vars.JSONParse(data) : data;
...
class MethodSnapshotContainer {
     ...
     */
    save(methodInstance) {
        const { name } = _function.getConfig(methodInstance);
        const { records, occupy, capacity } = this;
        if (name && occupy < capacity) {
            // 以method的name为key，将method实例保存到快照中
            const targetSnapshots = (records[name] = records[name] || _function.newInstance(SetCls));
    ...
    match(matcher, matchAll = true) {
        ...
        if (_function.isPlainObject(matcher)) {
            nameMatcher = matcher.name;
            matchHandler = matcher.filter;
        }
        if (_function.instanceOf(nameMatcher, vars.RegExpCls)) {
            nameReg = nameMatcher;
        }
        else if (_function.isString(nameMatcher)) {
            nameString = nameMatcher;
        }
        const { records } = this;
        // 通过解构的nameMatcher和filterHandler，获取对应的Method实例快照
        let matches = _function.newInstance((SetCls));
        // 如果有提供namespace参数则只在这个namespace中查找，否则在所有缓存数据中查找
        if (nameString) {
            matches = records[nameString] || matches;
        }
        else if (nameReg) {
            vars.forEach(vars.filterItem(vars.objectKeys(records), methodName => nameReg.test(methodName)), methodName => {
                records[methodName].forEach(method => matches.add(method));
            });
        }
        const fromMatchesArray = _function.isFn(matchHandler) ? vars.filterItem([...matches], matchHandler) : [...matches];
...
class Alova {
    constructor(options) {
        ...
        instance.snapshots = _function.newInstance((MethodSnapshotContainer), (_b = (_a = options.snapshots) !== null && _a !== void 0 ? _a : defaultAlovaOptions.snapshots) !== null && _b !== void 0 ? _b : 0);
    }
    Get(url, config) {
        return _function.newInstance((Method), typeGet, this, url, config);
    }
    Post(url, data, config) {
        return _function.newInstance((Method), typePost, this, url, config, data);
    }
    Delete(url, data, config) {
        return _function.newInstance((Method), typeDelete, this, url, config, data);
    }
    Put(url, data, config) {
        return _function.newInstance((Method), typePut, this, url, config, data);
    }
    Head(url, config) {
        return _function.newInstance((Method), typeHead, this, url, config);
    }
    Patch(url, data, config) {
        return _function.newInstance((Method), typePatch, this, url, config, data);
    }
    Options(url, config) {
        return _function.newInstance((Method), typeOptions, this, url, config);
    }
}
let boundStatesHook = vars.undefinedValue;
...
const createAlova = (options) => {
    const alovaInstance = _function.newInstance((Alova), options);
    ...
    !usingL1CacheAdapters.includes(l1Cache) && vars.pushItem(usingL1CacheAdapters, l1Cache);
    !usingL2CacheAdapters.includes(l2Cache) && vars.pushItem(usingL2CacheAdapters, l2Cache);
